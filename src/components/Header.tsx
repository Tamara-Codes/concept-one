    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-black/5">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="font-sans text-sm font-medium tracking-widest uppercase text-co-charcoal/70 hover:text-co-red transition-colors duration-300"
              >
                {link.label}
              </Link>
            ))}
          ...
          <a
            href={`tel:${site.phone}`}
            className="hidden lg:flex items-center gap-2 text-sm text-co-charcoal/60 hover:text-co-red transition-colors"
          >
            <PhoneIcon />
            {site.phoneDisplay}
          </a>
        ...
        <nav className="border-t border-black/5 bg-white px-6 py-4 flex flex-col gap-1">
          {navLinks.map((link) => (
            <Link
              ...
              className="font-sans text-sm font-medium tracking-widest uppercase text-co-charcoal/70 hover:text-co-red transition-colors py-2.5"
            >
              {link.label}
            </Link>
          ))}
          <div className="border-t border-black/5 mt-2 pt-3">
            <a
              href={`tel:${site.phone}`}
              className="inline-flex items-center gap-2 text-sm text-co-charcoal/60 hover:text-co-red transition-colors py-2"
            >
              <PhoneIcon />
              {site.phoneDisplay}
            </a>
